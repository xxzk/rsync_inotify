...
/usr/bin/inotifywait -mrq --format '%w%f %e' -e create,delete $monitor_path | while read line 
do
    ...
    if [ ${event} != 'DELETE' ];then
        rsync -avP --rsh=ssh -e "ssh -x -T -o Compression=no" ${file}  $rsync_user@$remote_server1::$rsync_module/ --password-file=$password_file --log-file=/var/log/rsync_inotify.log
        rsync -avP --rsh=ssh -e "ssh -x -T -o Compression=no" ${file}  $rsync_user@$remote_server2::$rsync_module/ --password-file=$password_file --log-file=/var/log/rsync_inotify.log
    fi
